<template>
    <el-form-item :label="itemData.label" :prop="itemData.key">
        <el-cascader
                :expand-trigger="itemData.expandTrigger"  v-if="isEdit"
...
<script>
    export default {
        ...
        computed:{
            ...
            showText(){
                let strArray=[]
                let valueKey=this.itemData.props.value
                let labelKey=this.itemData.props.label
                let childrenKey=this.itemData.props.children
                if(this.value&&this.value.length){
                    if(this.itemData.remote){
                        if(this.expandOptions){
                            for(let id of this.value){
                                let item=this.expandOptions[id]
                                if(item){
                                    strArray.push(item[labelKey])
                                }
                            }
                        }
                    }
                    else{
                        let findItems=this.itemData.options
                        for(let id of this.value){
                            let item=findItems.find(item=>item[valueKey]===id)
                            if(item){
                                strArray.push(item[labelKey])
                                findItems=item[childrenKey]
                            }
                        }
                    }
                }
