...
export default class Pagination extends Component {
  ...
  getPageHtml(i){
    return  <li key={i} className={this.state.current===i?"page currentPage":"page"} onClick={(e)=>this.click(i)}>{i}</li>
  }
  getPrevHtml(i){
    return <li key={i}  className="nextFive" onClick={()=>this.prev()}>...</li>
  }
  getNextHtml(i){
    return <li key={i} className="nextFive" onClick={()=>this.next()}>...</li>
  }
  left(i){
    if(i<=5||i==this.page){
